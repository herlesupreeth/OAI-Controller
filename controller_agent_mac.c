...
// #include "LAYER2/MAC/controller_agent_mac_proto.h"

#include "log.h"


/*Flags showing if a mac agent has already been registered*/
// unsigned int mac_agent_registered[NUM_MAX_CONTROLLER];

/*Array containing the Agent-MAC interfaces*/
// AGENT_MAC_xface *agent_mac_xface[NUM_MAX_CONTROLLER];

// int controller_agent_mac_handle_stats(mid_t mod_id, const void *params, Protocol__ProgranMessage **msg){
...
//   //eNB_MAC_INST *eNB = &eNB_mac_inst[ctrl_id];
//   //UE_list_t *eNB_UE_list=  &eNB->UE_list;
//   // report_config is not allocated memory here ..might get some errors
...
//       controller_agent_disable_cont_mac_stats_update(mod_id);
//       controller_agent_destroy_timer_by_task_id(xid);
//       *msg = NULL;
//       return 0;
...
// 	report_config.ue_report_type[i].ue_rnti = get_ue_crnti(ctrl_id, i); //eNB_UE_list->eNB_UE_stats[UE_PCCID(ctrl_id,i)][i].crnti;
...
// 	controller_agent_mac_stats_request(ctrl_id, xid, &request_config, &timer_msg);
// 	/* Create a timer */
// 	long timer_id = 0;
// 	controller_agent_timer_args_t *timer_args;
// 	timer_args = malloc(sizeof(controller_agent_timer_args_t));
// 	memset (timer_args, 0, sizeof(controller_agent_timer_args_t));
// 	timer_args->mod_id = ctrl_id;
...
// 	}
// 	controller_agent_create_timer(sec_interval, usec_interval, CONTROLLER_AGENT_DEFAULT, ctrl_id, CONTROLLER_AGENT_TIMER_TYPE_PERIODIC, xid, controller_agent_handle_timed_task,(void*) timer_args, &timer_id);
...
// 	request_config.period = 0;
// 	request_config.config = &report_config;
// 	controller_agent_enable_cont_mac_stats_update(ctrl_id, xid, &request_config);
//       }
//     }
//     break;
...
//   if (controller_agent_mac_stats_reply(ctrl_id, xid, &report_config, msg) < 0 ){
...
int controller_agent_mac_stats_request(mid_t mod_id,
				xid_t xid,
  ...
  case  PROTOCOL__PRP_STATS_TYPE__PRST_CELL_STATS:;
    ...
    cell_stats->cell = cells; // Changed by supreeth
    cell_stats->flags = report_config->config->cc_report_type[0].cc_report_flags; // Changed by supreeth
  ...
  case PROTOCOL__PRP_STATS_TYPE__PRST_UE_STATS:;
    ...
    ue_stats->rnti = ues; // Changed by supreeth
    ue_stats->flags = report_config->config->ue_report_type[0].ue_report_flags; // Changed by supreeth
  ...
  *msg = malloc(sizeof(Protocol__ProgranMessage));
  ...
  (*msg)->has_msg_dir = 1; //Added by supreeth
...
int controller_agent_mac_destroy_stats_request(Protocol__ProgranMessage *msg) {
...
int controller_agent_mac_print_stats_reply(mid_t mod_id, const void *params, Protocol__ProgranMessage **msg){
  ...
  *msg = (Protocol__ProgranMessage *)params;

  Protocol__ProgranMessage *input = (Protocol__ProgranMessage *)params;
  Protocol__PrpStatsReply *stats_reply_msg = input->stats_reply_msg;
  Protocol__PrpHeader *header = stats_reply_msg->header;
  ...
  printf("*********** HEADER ****************************\n");
  printf("xid --> %u\n", (stats_reply_msg->header)->xid);

  printf("*********** STATS REPLY ***********************\n");

  printf("n_ue_report --> %zu\n", stats_reply_msg->n_ue_report);
  printf("n_cell_report --> %zu\n", stats_reply_msg->n_cell_report);

  Protocol__PrpUeStatsReport **ue_report;
  Protocol__PrpCellStatsReport **cell_report;

  if (stats_reply_msg->n_ue_report > 0) {

    printf("*********** STATS REPLY - UE REPORT ***********************\n");

    ue_report = stats_reply_msg->ue_report;
    if (ue_report == NULL)
      goto error;
    for (i = 0; i < stats_reply_msg->n_ue_report; i++) {
      if(ue_report[i]->has_rnti)
        printf("rnti --> %u\n", ue_report[i]->rnti);
      if(ue_report[i]->has_flags)
        printf("flags --> %u\n", ue_report[i]->flags);

      /* Check the types of reports that need to be constructed based on flag values */

      /* Check flag for creation of buffer status report */
      if (ue_report[i]->flags & PROTOCOL__PRP_UE_STATS_TYPE__PRUST_BSR) {

        printf("*********** UE REPORT - PROTOCOL__PRP_UE_STATS_TYPE__PRUST_BSR ***********************\n");
        // progRAN specifications for more details

        printf("n_bsr --> %zu\n", ue_report[i]->n_bsr);

        printf("*********** Report for each LCG (4 elements) of current UE *********************\n");
        ...
        for (j = 0;j < ue_report[i]->n_bsr;j++) {
          ...
          printf("buffer status report --> %u\n", ue_report[i]->bsr[j]);
        }
      }

      /* Check flag for creation of PRH report */
      if (ue_report[i]->flags & PROTOCOL__PRP_UE_STATS_TYPE__PRUST_PRH) {

        printf("*********** UE REPORT - PROTOCOL__PRP_UE_STATS_TYPE__PRUST_PRH ***********************\n");

        printf("*********** Power headroom value for the RNTI *********************\n");

        if(ue_report[i]->has_phr)
          printf("power headroom report --> %u\n", ue_report[i]->phr);
      }

      /* Check flag for creation of RLC buffer status report */
      if (ue_report[i]->flags & PROTOCOL__PRP_UE_STATS_TYPE__PRUST_RLC_BS) {

        printf("*********** UE REPORT - PROTOCOL__PRP_UE_STATS_TYPE__PRUST_RLC_BS ***********************\n");

        // TODO: Fill in the actual RLC buffer status reports
        printf("n_rlc_report --> %zu\n", ue_report[i]->n_rlc_report); // Set this to the number of LCs for this UE
        Protocol__PrpRlcBsr ** rlc_reports;
        
        if (ue_report[i]->n_rlc_report > 0){
          rlc_reports = ue_report[i]->rlc_report;
          if (rlc_reports == NULL)
            goto error;
        }       
        
        // NN: see LAYER2/openair2_proc.c for rlc status
        printf("*********** Buffer status report for each logical channel of the UE *********************\n");

        for (j = 0; j < ue_report[i]->n_rlc_report; j++) {
          if (rlc_reports[j] == NULL)
            goto error;
      
          if(rlc_reports[j]->has_lc_id)
            printf("logical channel id --> %u\n", rlc_reports[j]->lc_id);
          if(rlc_reports[j]->has_tx_queue_size)
            printf("tx queue size in bytes --> %u\n", rlc_reports[j]->tx_queue_size);
          if(rlc_reports[j]->has_tx_queue_hol_delay)
            printf("tx queue head of line delay in ms --> %u\n", rlc_reports[j]->tx_queue_hol_delay);
          if(rlc_reports[j]->has_retransmission_queue_size)
            printf("retransmission queue size in bytes --> %u\n", rlc_reports[j]->retransmission_queue_size);
          if(rlc_reports[j]->has_retransmission_queue_hol_delay)
            printf("retransmission queue head of line delay in ms --> %u\n", rlc_reports[j]->retransmission_queue_hol_delay);
          if(rlc_reports[j]->has_status_pdu_size)
            printf("current size of the pending message in bytes --> %u\n", rlc_reports[j]->status_pdu_size);
        }
      }

      /* Check flag for creation of MAC CE buffer status report */
      if (ue_report[i]->flags & PROTOCOL__PRP_UE_STATS_TYPE__PRUST_MAC_CE_BS) {

        printf("*********** UE REPORT - PROTOCOL__PRP_UE_STATS_TYPE__PRUST_MAC_CE_BS ***********************\n");

        printf("*********** MAC CE buffer status report of the UE *********************\n");

        /* Use as bitmap. Set one or more of the; /* Use as bitmap. Set one or more of the PROTOCOL__PRP_CE_TYPE__PRPCET_ values
            found in stats_common.pb-c.h. See prp_ce_type in progRAN specification */
        if(ue_report[i]->has_pending_mac_ces)
          printf("MAC CE buffer status --> %u\n", ue_report[i]->pending_mac_ces);
      }

      /* Check flag for creation of DL CQI report */
      if (ue_report[i]->flags & PROTOCOL__PRP_UE_STATS_TYPE__PRUST_DL_CQI) {
        printf("*********** UE REPORT - PROTOCOL__PRP_UE_STATS_TYPE__PRUST_DL_CQI ***********************\n");

        printf("*********** DL CQI report for the UE *********************\n");

        Protocol__PrpDlCqiReport * dl_report;
        dl_report = ue_report[i]->dl_cqi_report;
        if (dl_report == NULL)
          goto error;

        if(dl_report->has_sfn_sn)
          printf("SFN and SF of the last report held in the agent --> %u\n", dl_report->sfn_sn);

        printf("number of DL CQI reports for this UE (One for each CC) --> %zu\n", dl_report->n_csi_report);

        //TODO:Create the actual CSI reports.
        Protocol__PrpDlCsi **csi_reports;
        if(dl_report->n_csi_report > 0){
          csi_reports = dl_report->csi_report;
          if (csi_reports == NULL)
            goto error;
        }
        ...
        for (j = 0; j < dl_report->n_csi_report; j++) {
          ...
          if(csi_reports[j]->has_serv_cell_index)
            printf("the servCellIndex for this report --> %u\n", csi_reports[j]->serv_cell_index);

          if(csi_reports[j]->has_ri)
            printf("the rank indicator value for this cc --> %u\n", csi_reports[j]->ri);
          ...
          //PROTOCOL__PRP_CSI_TYPE__PRCSIT_P10 is ONLY considered  // Needs work to be done

          if(csi_reports[j]->has_type)
            if(csi_reports[j]->type == PROTOCOL__PRP_CSI_TYPE__PRCSIT_P10)
              printf("*********** CSI report type PROTOCOL__PRP_CSI_TYPE__PRCSIT_P10 *********************\n");
          
          if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_P10CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_P10CSI *********************\n");
            Protocol__PrpCsiP10 *csi10;
            csi10 = csi_reports[j]->p10csi;
            if (csi10 == NULL)
              goto error;

            if(csi10->has_wb_cqi)
              printf("the wideband value (depends on cc_id) --> %u\n", csi10->wb_cqi);
          }
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_P11CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_P11CSI *********************\n");
          }
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_P20CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_P20CSI *********************\n");
          }
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_P21CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_P21CSI *********************\n");
          }
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_A12CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_A12CSI *********************\n");
          }
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_A22CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_A22CSI *********************\n");
          }
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_A20CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_A20CSI *********************\n");
          } 
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_A30CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_A30CSI *********************\n");
          }
          else if(csi_reports[j]->report_case == PROTOCOL__PRP_DL_CSI__REPORT_A31CSI){
            printf("*********** CSI report case PROTOCOL__PRP_DL_CSI__REPORT_A31CSI *********************\n");
          }
        }
      ...
      if (ue_report[i]->flags & PROTOCOL__PRP_UE_STATS_TYPE__PRUST_PBS) {

        printf("*********** UE REPORT - PROTOCOL__PRP_UE_STATS_TYPE__PRUST_PBS ***********************\n");

        printf("*********** Paging buffer status report ***********************\n"); //For this field to be valid, the RNTI set in the report must be a P-RNTI
        
        Protocol__PrpPagingBufferReport *paging_report;
        paging_report = ue_report[i]->pbr;
        if (paging_report == NULL)
          goto error;

        printf("number of pending paging messages --> %zu\n", paging_report->n_paging_info);
        printf("*********** A report for each pending paging message ***********************\n");
        Protocol__PrpPagingInfo **p_info;
        if(paging_report->n_paging_info > 0){
          p_info = paging_report->paging_info;
          if (p_info == NULL)
            goto error;          
        }
        ...
        for (j = 0; j < paging_report->n_paging_info; j++) {
          ...
          //paging index is the same that will be used for the scheduling of the paging message by the controller
          if(p_info[j]->has_paging_index)
            printf("paging index --> %u\n", p_info[j]->paging_index);
          if(p_info[j]->has_paging_message_size)
            printf("paging message size --> %u\n", p_info[j]->paging_message_size);
          if(p_info[j]->has_paging_subframe)
            printf("paging subframe --> %u\n", p_info[j]->paging_subframe);
          if(p_info[j]->has_carrier_index)
            printf("carrier index --> %u\n", p_info[j]->carrier_index);
        }
      }

      /* Check flag for creation of UL CQI report */
      if (ue_report[i]->flags & PROTOCOL__PRP_UE_STATS_TYPE__PRUST_UL_CQI) {

        printf("*********** UE REPORT - PROTOCOL__PRP_UE_STATS_TYPE__PRUST_UL_CQI ***********************\n");

        printf("*********** Full UL CQI report of the UE ***********************\n"); 

        Protocol__PrpUlCqiReport *full_ul_report;
        full_ul_report = ue_report[i]->ul_cqi_report;
        if(full_ul_report == NULL)
         goto error;

        if(full_ul_report->has_sfn_sn)
          printf("SFN and SF of the generated report --> %u\n", full_ul_report->sfn_sn);

        //number of UL measurement reports based on the types of measurements configured for this UE and on the servCellIndex
        printf("number of UL measurement reports --> %zu\n", full_ul_report->n_cqi_meas);
        Protocol__PrpUlCqi **ul_report;
        if (full_ul_report->n_cqi_meas > 0){
          ul_report = full_ul_report->cqi_meas;
          if(ul_report == NULL)
           goto error;
        }

        printf("*********** UL report of the UE for each of the configured report types ***********************\n");
        for(j = 0; j++; j < full_ul_report->n_cqi_meas) {
          if(ul_report[j] == NULL)
            goto error;

          //Type of the UL report. As an example set it to SRS UL report
          // See enum prp_ul_cqi_type in progRAN specification for more details // Needs work to be done

          // ul_report[j]->type = PROTOCOL__PRP_UL_CQI_TYPE__PRUCT_SRS;

          if(ul_report[j]->has_type)
            if(ul_report[j]->type == PROTOCOL__PRP_UL_CQI_TYPE__PRUCT_SRS)
              printf("*********** UL report type PROTOCOL__PRP_UL_CQI_TYPE__PRUCT_SRS *********************\n");

          //See struct prp_ul_cqi in progRAN specification for more details
          printf("number of SINR measurements based on the report type --> %zu\n", ul_report[j]->n_sinr); 
          uint32_t *sinr_meas;
          if (ul_report[j]->n_sinr > 0){
            sinr_meas = ul_report[j]->sinr;
            if (sinr_meas == NULL)
              goto error;          
          }

          for (k = 0; k < ul_report[j]->n_sinr; k++) {
            //printf("SINR measurements for the specified type --> %u\n", sinr_meas[k]); //Commented for using constant value
          }

          printf("SINR measurements for the specified type --> %u\n", sinr_meas[0]);

          if(ul_report[j]->has_serv_cell_index)
            printf("servCellIndex for this report --> %u\n", ul_report[j]->serv_cell_index);
        }
      }
      printf("*********** STATS REPLY - UE %d REPORT ***********************\n", i);
      printf("\n");
      printf("\n");
    }
  }
  ...
  if (stats_reply_msg->n_cell_report > 0) {
    cell_report = stats_reply_msg->cell_report;
    if (cell_report == NULL)
      goto error;    

    printf("*********** STATS REPLY - CELL REPORT ***********************\n");

    for (i = 0; i < stats_reply_msg->n_cell_report; i++) {
      if(cell_report[i] == NULL)
        goto error;

      if(cell_report[i]->has_carrier_index)
        printf("Cell Carrier Index --> %u\n", cell_report[i]->carrier_index);

      if(cell_report[i]->has_flags)
        printf("Flags --> %u\n", cell_report[i]->flags);

      /* Check flag for creation of noise and interference report */
      if(cell_report[i]->flags & PROTOCOL__PRP_CELL_STATS_TYPE__PRCST_NOISE_INTERFERENCE) {
        printf("*********** CELL REPORT - PROTOCOL__PRP_CELL_STATS_TYPE__PRCST_NOISE_INTERFERENCE ***********************\n");

        printf("*********** noise and interference report for this cell ***********************\n");
        Protocol__PrpNoiseInterferenceReport *ni_report;
        ni_report = cell_report[i]->noise_inter_report;
        if(ni_report == NULL)
          goto error;

        if(ni_report->has_sfn_sf)
          printf("Current frame and subframe number --> %u\n", ni_report->sfn_sf);

        if(ni_report->has_rip)
          printf("Received interference power in dbm --> %u\n", ni_report->rip);

        if(ni_report->has_tnp)
          printf("Thermal noise power in dbm --> %u\n", ni_report->tnp);
      }
    }
  }
...
// int controller_agent_mac_stats_reply(mid_t mod_id,
// 			      xid_t xid,
...
// 	  // Set the actual BSR for LCG j of the current UE
// 	  // NN: we need to know the cc_id here, consider the first one
// 	  elem[j] = get_ue_bsr (ctrl_id, i, j); //eNB_UE_list->UE_template[UE_PCCID(ctrl_id,i)][i].bsr_info[j];
// 	}
...
// 	ue_report[i]->phr = get_ue_phr (ctrl_id, i); // eNB_UE_list->UE_template[UE_PCCID(ctrl_id,i)][i].phr_info;
...
// 	  rlc_reports[j]->tx_queue_size = get_tx_queue_size(ctrl_id,i,j+1);
...
// 	ue_report[i]->pending_mac_ces = (get_MAC_CE_bitmap_TA(ctrl_id,i) | (0 << 1) | (0 << 2) | (0 << 3)) & 15; /* Use as bitmap. Set one or more of the; /* Use as bitmap. Set one or more of the
...
// 	  csi_reports[j]->ri = get_current_RI(ctrl_id,i,j);
...
// 			  csi10->wb_cqi = get_ue_wcqi (ctrl_id, i); //eNB_UE_list->eNB_UE_stats[UE_PCCID(ctrl_id,i)][i].dl_cqi;
// 			  csi10->has_wb_cqi = 1;
...
// 	full_ul_report->sfn_sn = get_sfn_sf(ctrl_id);
...
// 	ni_report->sfn_sf = get_sfn_sf(ctrl_id);
...
int controller_agent_mac_destroy_stats_reply(Protocol__ProgranMessage *msg) {
...
// int controller_agent_mac_sr_info(mid_t mod_id, const void *params, Protocol__ProgranMessage **msg) {
//   Protocol__PrpHeader *header;
//   int i;
...
int controller_agent_mac_destroy_sr_info(Protocol__ProgranMessage *msg) {
...
// int controller_agent_mac_sf_trigger(mid_t mod_id, const void *params, Protocol__ProgranMessage **msg) {
//   Protocol__PrpHeader *header;
//   int i,j;
//   const int xid = *((int *)params);
...
// Not needed but uncommented for compiling purposes
int controller_agent_mac_destroy_sf_trigger(Protocol__ProgranMessage *msg) {
  int i;
...
// int controller_agent_mac_create_empty_dl_config(mid_t mod_id, Protocol__ProgranMessage **msg) {
...
// Not needed but uncommented for compiling purposes
int controller_agent_mac_destroy_dl_config(Protocol__ProgranMessage *msg) {
  int i,j, k;
...
//   err_code = controller_agent_mac_sr_info(mod_id, (void *) &xid, &msg);
//   if (err_code < 0) {
//     goto error;
//   }
...
//     if (controller_agent_msg_send(mod_id, CONTROLLER_AGENT_MAC, data, size, priority)) {
...
//   }
//  error:
//   LOG_D(CONTROLLER_AGENT, "Could not send sr message\n");
// }
...
//   /*TODO: Must use a proper xid*/
//   err_code = controller_agent_mac_sf_trigger(mod_id, (void *) &xid, &msg);
//   if (err_code < 0) {
//     goto error;
//   }
...
//   }
//  error:
//   LOG_D(CONTROLLER_AGENT, "Could not send sf trigger message\n");
// }
...
//   err_code = controller_agent_mac_handle_stats(mod_id, (void *) mac_stats_context[mod_id].stats_req, &current_report);
//   if (err_code < 0) {
//     goto error;
//   }
...
//     msg = controller_agent_generate_diff_mac_stats_report(current_report, mac_stats_context[mod_id].prev_stats_reply);

//     /*Destroy the old stats*/
//      controller_agent_destroy_progran_message(mac_stats_context[mod_id].prev_stats_reply);
//   }
...
// int controller_agent_register_mac_xface(mid_t mod_id, AGENT_MAC_xface *xface) {
...
//   xface->controller_agent_send_sr_info = controller_agent_send_sr_info;
//   xface->controller_agent_send_sf_trigger = controller_agent_send_sf_trigger;
//   xface->controller_agent_send_update_mac_stats = controller_agent_send_update_mac_stats;
//   xface->controller_agent_schedule_ue_spec = schedule_ue_spec_default;
...
// int controller_agent_unregister_mac_xface(mid_t mod_id, AGENT_MAC_xface *xface) {

//   //xface->agent_ctxt = NULL;
//   xface->controller_agent_send_sr_info = NULL;
//   xface->controller_agent_send_sf_trigger = NULL;
//   xface->controller_agent_send_update_mac_stats = NULL;
//   xface->controller_agent_schedule_ue_spec = NULL;
...
/******************************************************
 *Implementations of controller_agent_mac_internal.h functions
 ******************************************************/
// // Needed
// err_code_t controller_agent_init_cont_mac_stats_update(mid_t mod_id) {
...
// err_code_t controller_agent_destroy_cont_mac_stats_update(mid_t mod_id) {
//   /*Disable the continuous updates for the MAC*/
//   mac_stats_context[mod_id].cont_update = 0;
//   mac_stats_context[mod_id].is_initialized = 0;
//   controller_agent_destroy_progran_message(mac_stats_context[mod_id].stats_req);
//   controller_agent_destroy_progran_message(mac_stats_context[mod_id].prev_stats_reply);
...
// err_code_t controller_agent_enable_cont_mac_stats_update(mid_t mod_id,
...
//   controller_agent_mac_stats_request(mod_id, xid, stats_req, &req_msg);
...
//  error:
//   LOG_E(CONTROLLER_AGENT, "mac_stats_context for eNB %d is not initialized\n", mod_id);
//   return -1;
// }

// err_code_t controller_agent_disable_cont_mac_stats_update(mid_t mod_id) {
//   /*Disable the continuous updates for the MAC*/
//   if (pthread_mutex_lock(mac_stats_context[mod_id].mutex)) {
//     goto error;
//   }
